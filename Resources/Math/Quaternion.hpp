...
namespace std {
template<>
struct hash<Stitches::Quaternion> {
	size_t operator()(const Stitches::Quaternion &quaternion) const noexcept {
		size_t seed = 0;
        Stitches::Maths::HashCombine(seed, quaternion.x);
        Stitches::Maths::HashCombine(seed, quaternion.y);
        Stitches::Maths::HashCombine(seed, quaternion.z);
        Stitches::Maths::HashCombine(seed, quaternion.w);
		return seed;
	}
};
}
