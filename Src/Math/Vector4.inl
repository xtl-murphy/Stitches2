...
namespace std {
template<typename T>
struct hash<Stitches::Vector4<T>> {
	size_t operator()(const Stitches::Vector4<T> &vector) const noexcept {
		size_t seed = 0;
        Stitches::Maths::HashCombine(seed, vector.x);
        Stitches::Maths::HashCombine(seed, vector.y);
        Stitches::Maths::HashCombine(seed, vector.z);
        Stitches::Maths::HashCombine(seed, vector.w);
		return seed;
	}
};
NS_STITCHES_END
