...
namespace std {
template<>
struct hash<Stitches::Matrix4> {
	size_t operator()(const Stitches::Matrix4 &matrix) const noexcept {
		size_t seed = 0;
        Stitches::Maths::HashCombine(seed, matrix[0]);
        Stitches::Maths::HashCombine(seed, matrix[1]);
        Stitches::Maths::HashCombine(seed, matrix[2]);
        Stitches::Maths::HashCombine(seed, matrix[3]);
		return seed;
	}
};
}
